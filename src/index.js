...
const debug = require('debug')('catalog-creator')


const pluginsDir = process.env.PLUGINS  || "./plugins"
debug(`plugins path is ${pluginsDir}`);
const TemplateFile = path.resolve(__dirname, "template.md")
...
const createMD = (template , data)=>{
  ...
  let pluginData = plugins.flatMap((plugin)=>{
    ...
    pluginMeta.name = path.basename(plugin);
    ...
    let template = kefir.fromNodeCallback(_.partial(fs.readFile, TemplateFile))
